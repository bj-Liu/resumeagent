...
from utils import write_file,get_content_between_a_b,wrap_func,cal_cost,extract_img_from_html,create_file,extract,modify_input_dict,extract_dimensions,get_html_css_from_response,get_all_files_in_dir
...
class HomepageDesignAgent(BaseAgent):
    def __init__(self, model="gpt4o-0513",save_file = "saves/",**kwargs) -> None:
        super().__init__(model)
        self.task = {"img":None,"basic_information":None, "academic_achievements":None, "experience":None, "professional_skills":None}
        self.task_queue = []
        self.memory = []
    ...
    def change_save_file(self,save_file):   
        ...
        print(f"Attempting to create directory: {save_file}")
        if not os.path.exists(save_file):
            os.makedirs(save_file)
    ...
    def publish_task(self,img = None, basic_information = None, academic_achievements = None, experience=None, professional_skills=None,feedback=None,refine_times= None, max_cost_prompt_tokens = 1000000,max_cost_completion_tokens = 1000000):
        self.task = {"img":img,"basic_information":basic_information, "academic_achievements":academic_achievements, "experience":experience, "professional_skills":professional_skills, "feedback":feedback}
    ...
    def plan(self):
        ...
        basic_information = self.task["basic_information"]    
        academic_achievements = self.task["academic_achievements"]
        experience = self.task["experience"]
        professional_skills = self.task["professional_skills"]
        img = self.task["img"]
        feedback = self.user_feedback if self.user_feedback else ""
        prompt = get_plan_prompt(basic_information=basic_information,
    ...
    def refine_page(self,page_info,feedback = ""):
        ...
        basic_information = self.task.get("basic_information", "")
        academic_achievements = self.task.get("academic_achievements", "")
        experience = self.task.get("experience", "")
        professional_skills = self.task.get("professional_skills", "")

      
        combined_text = (
            f"Basic Information: {basic_information}\n"
            f"academic_achievements: {academic_achievements}\n"
    ...
    def get_write_original_website_messages(self,page_info):
        ...
        professional_skills = self.task["professional_skills"]
        feedback = self.user_feedback if self.user_feedback else ""
        prompt = get_write_original_website_prompt(basic_information=basic_information, academic_achievements=academic_achievements, experience=experience, professional_skills=professional_skills,img=img,page_info=page_info,css_frame=self.css_frame,feedback=feedback,language=self.language)
        if img:     
            messages = [
                {"role":"user","content":[
                    {"type":"image","url":img},
    ...
    def get_refine_messages(self,page_info):   
        ...
        #img = self.task["img"] if page_info["is_main_page"] else None
        img = self.task["img"] if page_info.get("is_main_page", False) else None
        basic_information = self.task["basic_information"] 
        academic_achievements = self.task["academic_achievements"]
        experience = self.task["experience"]
        professional_skills = self.task["professional_skills"]
        ...
        if len(self.local_img_storage) > 10:    
            ...
            local_img_storage = self.local_img_storage
        prompt = get_refine_prompt(basic_information=basic_information, academic_achievements=academic_achievements, experience=experience, professional_skills=professional_skills,img=img,html_code=html_code,css_code=css_code,feedback=feedback,page_info=page_info,css_frame=css_frame,language=self.language,local_img_storage=local_img_storage,vision=self.vision)
    ...
    def delete_all_backups(self,directory):
        backup_folder = os.path.join(directory, "backup")  
        try:
            if os.path.exists(backup_folder):
                shutil.rmtree(backup_folder) 
                print(f"已删除备用文件夹及其内容: {backup_folder}")
            else:
                print(f"备用文件夹 {backup_folder} 不存在。")
        except Exception as e:
            print(f"删除备用文件夹时出错: {e}")
    ...
    def complete_page(self,pages,idx):  
        ...
        task = combined_text if combined_text else ""
    ...
    async def add_img_to_storage_async(self,img_path):
        ...
        if not response:
            print("Failed to get the response.")
            return
        chinese_description = extract(response,"zh")
        english_description = extract(response,"en")
    ...
    def clean(self):   
        ...
        self.delete_all_backups(self.save_file)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()  
    parser.add_argument("--save_file", type=str, default="saves/shopping/")
    parser.add_argument("--basic_information", type=str, default=None)
    parser.add_argument("--academic_achievements", type=str, default=None)
    parser.add_argument("--experience", type=str, default=None)
    parser.add_argument("--professional_skills", type=str, default=None)
    parser.add_argument("--img", type=str, default=None)
    parser.add_argument("--refine_times", type=int, default=2)
    
    args = parser.parse_args()
    save_file = args.save_file
    basic_information = args.basic_information
    academic_achievements = args.academic_achievements
    experience = args.experience
    professional_skills = args.professional_skills
    img = args.img
    refine_times = args.refine_times
    agent = HomepageDesignAgent(save_file=save_file)
    agent.act(basic_information=basic_information, academic_achievements=academic_achievements, experience=experience, professional_skills=professional_skills,img = img,refine_times = refine_times)
